...
import { Building_Base_Resource_Requirement_Form } from "~/app/derivean/root/building/Building_Base_Resource_Requirement_Form";

export namespace Building_Base_Resource_Requirement_Table {
...
const column = withColumn<Building_Base_Resource_Requirement_Table.Data>();
					...
					return (
						<ActionMenu>
							<ActionModal
								...
								icon={ResourceIcon}
							>
								<Building_Base_Resource_Requirement_Form
									mutation={useMutation({
										async mutationFn(values) {
											return toast.promise(
												kysely.transaction().execute(async (tx) => {
													return tx
														.insertInto("Building_Base_Resource_Requirement")
														.values({
															id: genId(),
															...values,
															buildingBaseId,
									...
									mutation={useMutation({
										async mutationFn(values) {
											return toast.promise(
												kysely.transaction().execute(async (tx) => {
													return tx
														.updateTable("Building_Base_Resource_Requirement")
														.set(values)
														.where("id", "=", data.id)
														.returningAll()
														.executeTakeFirstOrThrow();
								...
								<DeleteControl
									callback={async () => {
										return kysely.transaction().execute(async (tx) => {
											return tx
												.deleteFrom("Building_Base_Resource_Requirement")
												.where("id", "=", data.id)
												.execute();
										});
									}}
