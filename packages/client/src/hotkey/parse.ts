...
export function parse(hotkey: string): Hotkey {
    ...
    return {
        ...
        key: keys.find((key) => !["alt", "ctrl", "meta", "shift", "mod"].includes(key)),
    };
}
...
function isHotkey(hotkey: Hotkey, event: KeyboardEvent): boolean {
    ...
    return !!(key && (pressedKey.toLowerCase() === key.toLowerCase() || event.code.replace("Key", "").toLowerCase() === key.toLowerCase()));
}
