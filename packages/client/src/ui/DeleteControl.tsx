...
export namespace DeleteControl {
	...
	export interface Props<
		...
		TCollectionMutationSchema extends CollectionMutationSchema<
			ShapeSchema,
			TQuerySchema
		>,
	> {
...
>({
	...
	const blockStore = useStore(BlockStore, ({ block, unblock }) => ({
		block,
		unblock,
	}));
	const queryStore = withQueryStore.useSelector(({ where, filter }) => ({
		where,
		filter,
	}));
	...
	return (
		<div>
			...
			<div
				className={cssOf(
					"flex flex-row items-center justify-between mt-4 pt-4 border-t border-t-slate-100",
				)}
			>
				<Button
					icon={{
				...
				<Button
					...
					onClick={() => {
						...
						mutation.mutate(
							...
							{
								onSuccess: () => {
									toast.success(t()`Item deleted (toast)`);
								},
								onSettled: () => {
									close();
									setTimeout(() => {
