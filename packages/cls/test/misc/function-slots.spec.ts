...
describe("function-style slots", () => {
	it("slots return functions that can be called with variant overrides", () => {
		...
		// Test with variant overrides (CreateConfig)
		expect(
			slots.root({
				variant: {
					size: "sm",
		...
		// Test full CreateConfig at slot call-time
		expect(
			slots.root({
				variant: {
					size: "sm",
	...
	it("function-style slots work with create-time overrides", () => {
		...
		// Per-slot full config: add per-call slot appends
		expect(
			slots.root({
				slot: {
					root: {
		...
		).toBe("inline-flex items-center px-4 py-2 bg-blue-600 ring-1");
	});
